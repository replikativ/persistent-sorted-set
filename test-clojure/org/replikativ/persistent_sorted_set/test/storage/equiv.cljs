(ns org.replikativ.persistent-sorted-set.test.storage.equiv
       ...
       (do
         (testing "restored empty set"
           (let [async-storage (async-storage)
             ...
             (is (uuid? sync-address) "precondition: store returned an address")
             (is (uuid? async-address) "precondition: store returned an address")
             (do
